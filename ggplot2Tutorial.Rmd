...
```{r,eval=FALSE}
p + geom_area(aes(fill=variable, x=value),stat='bin')
p + aes(x=value)+stat_bin(aes(fill=variable),geom='bar')
```
...
## 标度工作原理
- 定义域
    - 离散：variable={A, T, G, C, N}
    - 连续：value=[0, 48.6]
- 值域
    - 离散：scale_colour\_discrete(variable)={red, green, blue, yellow, purple}
    - 连续：scale_colour\_continue(value)=[white -> blue]
- 映射过程
    - 变换
    - 训练
    - 映射
...
head(base_qual)
ggplot(base_qual,aes(x=Pos,y=Qual)) +

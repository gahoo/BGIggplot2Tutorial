...
sidebarLayout(
  sidebarPanel(
    selectInput('coord_dataset', label='dataset:',
                choices = c('base_comp'),
                selected = 'base_comp'),
    selectInput('coord_func_name', label='function:',
...
src_coord = reactive({
  args<-getCtrlsArgs(coord_func[[input$coord_func_name]])
  args<-paste(names(args), args, sep=' = ', collapse=', ')
  sprintf('ggplot(base_comp,aes(x=Pos,y=value,colour=variable)) +
  geom_point() +
  %s(%s)',input$coord_func_name, args
   )})
